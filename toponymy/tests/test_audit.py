...
class MockClusterLayer:
    ...
    def __init__(self, n_clusters=5, layer_id=0):
        ...
        # Simulate keyphrases
        self.keyphrases = [
            [f"keyword_{i}_{j}" for j in range(5)] for i in range(n_clusters)
        ]

        # Simulate exemplars
        self.exemplars = [
            [f"Example doc {j} for cluster {i}" for j in range(3)]
        ...
        if layer_id > 0:
            self.subtopics = [
                [f"Subtopic {i}.{j}" for j in range(2)] for i in range(n_clusters)
            ]
        else:
            self.subtopics = None
...
class MockToponymyModel:
    ...
    def __init__(self, n_layers=3):
        self.cluster_layers_ = [
            MockClusterLayer(n_clusters=10 - i * 3, layer_id=i) for i in range(n_layers)
        ]
...
class TestAuditFunctions:
    ...
    def test_get_cluster_documents(self):
        ...
        cluster_docs = get_cluster_documents(
            self.mock_model, layer_index=0, cluster_id=0, original_texts=documents
        )
