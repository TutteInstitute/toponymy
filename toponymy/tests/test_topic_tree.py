...
def test_topic_tree_string():
    ...
    result = topic_tree_string_recursion(
        tree_dict, (3, 0), topics, topic_sizes, n_objects
    )
    print(result)
    expected_result = (
        "Topic tree:\n"
...
def test_topic_tree_html_with_colors():
    ...
    result = topic_tree_html(
        tree_dict, topics, topic_sizes, n_objects, variable_color=True
    )
    expected_result = """<div class="topic-tree">
    <style>
        .topic-tree ul {
...
def test_topic_tree_class():
    ...
    tree_instance = TopicTree(
        tree_dict, topics, topic_sizes=topic_sizes, n_objects=n_objects
    )
    ...
    assert tree_instance._repr_html_() == topic_tree_html(
        tree_dict, topics, topic_sizes, n_objects
    )
