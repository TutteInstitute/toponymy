...
MATRIX, KEYPHRASES = build_object_x_keyphrase_matrix(ALL_TOPIC_OBJECTS, ngram_range=(1,1), token_pattern=r"(?u)\b\w\w+\b")
...
def test_bm25_keyphrases():
    ...
    scoring = [[retriever.get_scores([x]) for x in bm25_results[i]] for i in range(len(bm25_results))]
    ...
    for i in range(len(scoring)):
        ...
        for j in range(len(scoring)):
            if i == j:
                continue
            assert min(score_for_cluster) >= max([x[i] for n, x in enumerate(scoring[j]) if bm25_results[j][n] not in bm25_results[i]])
