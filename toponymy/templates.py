...
{%- endfor %}
{%- endif %}
{%- if cluster_history[loop.index - 1] %}
- In earlier iterations of the dataset, the names of this topic in order of recency (from most to least) include:
{%- for subtopic in cluster_subtopics["misc"][loop.index - 1] %}
    * {{subtopic}}
{%- endfor %}
{%- endif %}
...
{%- if cluster_history[loop.index - 1] %}
- In earlier iterations of the dataset, the names of this topic in order of recency (from most to least) include:
{%- for subtopic in cluster_history[loop.index - 1] %}
    * {{subtopic}}
{%- endfor %}
{%- endif %}
{% endfor %}
...
"""
    ...
    "harmonize_temporal": {
        "system": jinja2.Template(
            """
You are an expert in {{larger_topic}}. You have been asked to compare topic names assigned to sets of {{document_type}} from {{corpus_description}} that have been assigned to related topics at different points in time.

Your task is to determine if the previous name for each topic group is still appropriate for the topic group. If the current and previous topic names mean the same or very similar things, then the previous topic name is still appropriate, otherwise it is not. 

The response must be formatted as a single JSON object in the format:
{"new_topic_name_mapping": {"1. OLD_NAME1": "NEW_NAME1", "2. OLD_NAME2": "NEW_NAME2", ... }, "topic_specificities": [NEW_TOPIC_SCORE1, NEW_TOPIC_SCORE2, ...]}
where SCORE is a float value between 0.0 and 1.0 representing how appropriate you believe the old name is for the current topic, and if SCORE>=0.8 then the new name should equal the old name.
