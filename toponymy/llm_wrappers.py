...
try:
    ...
    class VLLMNamer(LLMWrapper):
        ...
        **kwargs: dict, optional
        ...
        def _call_llm_with_system_prompt_and_json_schema(
            self, system_prompt: str, user_prompt: str, temperature: float, max_tokens: int, json_schema: str
        ) -> str:
        ...
        def _call_llm_with_system_prompt(
            self, system_prompt: str, user_prompt: str, temperature: float, max_tokens: int, json_schema: Optional[str] = None
        ) -> str:
            messages = [
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": user_prompt + self.extra_prompting},
    ...
    class AsyncVLLMNamer(AsyncLLMWrapper):
        ...
        async def _call_llm_batch(
            self, prompts: List[str], temperature: float, max_tokens: int, json_schema: Optional[str] = None
        ) -> List[str]:
            messages = [
                [{"role": "user", "content": prompt + self.extra_prompting}]
                for prompt in prompts
