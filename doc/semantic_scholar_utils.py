...
class SemanticScholarAPI:
    ...
    @retry(
        ...
        wait=wait_exponential(multiplier=1, min=1, max=60),  # Increased max wait for 429 errors
    ...
    def _is_cached(self, table: str, key: str) -> bool:
        ...
        if table == 'authors':
            ...
            cursor.execute('SELECT 1 FROM citations WHERE cache_key = ? LIMIT 1', (key,))
        else:
            conn.close()
            return False
    ...
    def _get_cached(self, table: str, key: str) -> Optional[Union[dict, list]]:
        ...
        if table == 'authors':
            ...
            cursor.execute('SELECT citing_papers FROM citations WHERE cache_key = ?', (key,))
        else:
            return None
    ...
    def get_author_papers(self, author_id: str, max_papers: Optional[int] = None, 
                         include_citations: bool = True) -> List[Dict]:
        """
    ...
    def get_paper_citations(self, paper_id: str, max_citations: Optional[int] = None,
        ...
        self._cache('citations', cache_key, citing_papers, 
                   paper_id=paper_id, year_filter=year_filter)
        
        return citing_papers
    
    def is_citations_cached(self, paper_id: str, citation_count: int,
        ...
        effective_count = min(citation_count, max_citations) if max_citations else citation_count
    ...
    def get_paper_citations_with_time_slicing(self, paper_id: str, citation_count: int,
                                              max_citations: Optional[int] = None,
                                              current_year: Optional[int] = None) -> List[Dict]:
        """
        ...
        while collected < effective_citation_count:
            ...
            if len(citations) >= 10000:
                print(f"  ⚠ WARNING: Paper has >=10,000 citations in {year} alone - some may be truncated")
    ...
    def batch_get_papers(self, paper_ids: List[str], progress: bool = True) -> List[Dict]:
        """
        ...
        if uncached_ids:
            ...
            for i in iterator:
                ...
                batch_papers = self._make_request('POST', url, params=params, json=payload)
...
def build_citation_network(
    ...
    if verbose:
        ...
        if api_key:
            ...
            print(f"  Get an API key at https://www.semanticscholar.org/product/api for faster queries")
    ...
    author_papers = api.get_author_papers(author_id, max_papers=max_author_papers, 
    ...
    for paper in author_papers:
        ...
        if inline_citations:
            ...
            if len(inline_citations) < citation_count and max_citations_per_paper is None:
                ...
                for idx, citation in enumerate(inline_citations):
                    if max_citations_per_paper is not None and idx >= max_citations_per_paper:
                        break
    ...
    if total_inline_citations == 0 and papers_needing_full_query:
        ...
        print(f"  {len(papers_needing_full_query)} papers need separate queries ({total_citations_to_fetch:,} total citations)")
    elif verbose:
    ...
    if papers_needing_full_query:
        ...
        for idx, (paper_id, citation_count) in enumerate(papers_needing_full_query, 1):
            if verbose:
                ...
                is_cached = api.is_citations_cached(paper_id, citation_count, max_citations_per_paper)
                ...
                print(f"  [{idx}/{len(papers_needing_full_query)}] Fetching citations for paper with {citation_count:,} citations{warning}{cached_label}")
            ...
            if max_total_citing_papers is not None and len(all_citing_papers) >= max_total_citing_papers:
                if verbose:
                    print(f"  Reached limit of {max_total_citing_papers} total citing papers")
                break
    ...
    papers_needing_fetch = [pid for pid, p in all_citing_papers.items() if 'title' not in p]
